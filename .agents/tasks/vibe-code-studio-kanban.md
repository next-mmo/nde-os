...
- [ ] **1. Tabs, Layout & Chat Integration**
  - [ ] 1.1 Add `Kanban` into `VibeCodeStudio` tabs enum.
  - [ ] 1.2 Create `VibeCodeStudio/tabs/KanbanBoard.svelte` as the root component.
  - [ ] 1.3 Update Chat Panel header to include a mode toggle (`Figma Agent` | `Scrum Master`).
  - [ ] 1.4 Route chat prompts with distinct system prompts based on the active mode.
  - [ ] 1.5 Implement an autocomplete popup in the Chat input triggered by `/` or `@` for Skills/MCP mentions.

- [ ] **2. Tauri Backend Commands & File Watcher**
  - [ ] 2.1 Update/Create Tauri command `get_agent_tasks` to scan `.agents/tasks/*.md` and extract metadata (filename, title, status).
  - [ ] 2.2 Update/Create Tauri command `update_agent_task_status` to safely overwrite the status header.
  - [ ] 2.3 Spin up a Tauri file watcher on `.agents/tasks` directory. Emit standard `tasks://updated` events to frontend.

- [ ] **3. UI & State Implementation**
  - [ ] 3.1 Create a frontend state proxy `kanban.svelte.ts` that syncs strictly via the Tauri IPC / file watcher.
  - [ ] 3.2 Create generic Kanban Columns representing (`Plan`, `YOLO`, `Done`, `Verified`, `Reopen`).
  - [ ] 3.3 Create Kanban Card component with lock-status UI.
  - [ ] 3.4 Implement Drag-and-Drop library/native API (compatible with Svelte 5).

- [ ] **4. Real-time & Locking Mechanism**
  - [ ] 4.1 If status matches `🟡 yolo mode`, tag task state as `locked: true`.
  - [ ] 4.2 Disable `draggable` attribute for locked cards. Show a padlock visual.

- [ ] **5. Polish & Styling**
  - [ ] 5.1 Use shadcn-svelte aesthetics. Glassmorphism for kanban cards, dark mode accents.
  - [ ] 5.2 Validate cross-platform pathing for reading/writing `tasks/*.md`.
