...
/// Recursively scan for video files in the sandbox.
#[tauri::command]
pub async fn scan_videos(state: State<'_, AppState>) -> Result<Vec<FileEntry>, String> {
    let base_dir = state.base_dir.clone();
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let video_exts = ["mp4", "mkv", "avi", "mov", "webm", "m4v", "wmv", "flv"];
