...
const APP_ID = "sample-node-fullstack";
...
test.describe.serial("Node.js Fullstack Counter App", () => {
  ...
  test("installs and launches into a browser window", async ({ page }) => {
    ...
    await expect(row.getByRole("button", { name: "Open in Window" })).toBeVisible({ timeout: 180000 });
    ...
    await expect(browserWindow.getByRole("textbox", { name: "Browser address" })).toHaveValue(/localhost:3000/);
  });

  test("full lifecycle: overview to install to running to uninstall", async ({ page }) => {
    await openLauncher(page);
    ...
    const detailPanel = page.locator(".detail-panel", { has: page.locator("h3", { hasText: APP_NAME }) });
    await expect(detailPanel.getByRole("button", { name: "Open External" })).toBeVisible();
    ...
    await expect(page.getByText("Install an app from the catalog to see it here.")).toBeVisible({ timeout: 15000 });
  });
});
