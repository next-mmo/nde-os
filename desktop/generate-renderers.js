...
const mappings = {
  Card: `<script lang="ts">
  let p = $props();
  const padMap = { none: "0", sm: "0.5rem", md: "1rem", lg: "1.5rem" };
...
<div style="border-radius:1rem;padding:{padding};{variantStyle}">
  ...
  let p = $props();
  const gapMap = { none: "0", xs: "0.25rem", sm: "0.5rem", md: "0.75rem", lg: "1.25rem", xl: "2rem" };
  const alignMap = { start: "flex-start", center: "center", end: "flex-end", stretch: "stretch" };
  const justMap = { start: "flex-start", center: "center", end: "flex-end", between: "space-between", around: "space-around" };
</script>
...
{/if}`,
  ...
  let p = $props();
  const headingSize = { "1": "1.5rem", "2": "1.2rem", "3": "1rem", "4": "0.88rem" };
...
<svelte:element this={tag} style="margin:0;font-size:{headingSize[level]};font-weight:600{p.muted ? ';color:var(--system-color-text-muted)' : ''}">
  ...
  Text: `<script lang="ts">
  let p = $props();
  const fontSizeMap = { xs: "0.72rem", sm: "0.82rem", md: "0.92rem", lg: "1.1rem" };
  let styles = $derived([
...
<span style="display:inline-block;padding:0.15rem 0.5rem;border-radius:999px;font-size:0.7rem;font-weight:500;background:{c.bg};color:{c.text}">{p.text}</span>`,
  ...
  let p = $props();
  const trendColor = { up: "hsl(145 65% 55%)", down: "hsl(0 70% 60%)", neutral: "var(--system-color-text-muted)" };
...
<div style="display:flex;flex-direction:column;gap:0.15rem">
  ...
  let p = $props();
  const btnSizeMap = { sm: "0.35rem 0.7rem", md: "0.5rem 1rem", lg: "0.65rem 1.3rem" };
  const btnVariants = {
...
<div style="display:flex;align-items:center;gap:0.75rem;padding:0.75rem;border-radius:0.85rem;border:1px solid var(--system-color-border);background:var(--system-color-panel);cursor:pointer;transition:border-color 0.15s">
  <span style="font-size:1.5rem">{p.icon ?? "í³¦"}</span>
...
for(let [name, code] of Object.entries(mappings)) {
