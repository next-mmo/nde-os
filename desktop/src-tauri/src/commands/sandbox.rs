...
#[tauri::command]
pub fn verify_sandbox(state: State<'_, AppState>, app_id: String) -> Result<SandboxVerifyResult, String> {
    let mgr = state.manager.lock().map_err(|e| e.to_string())?;
    mgr.verify_sandbox(&app_id).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn get_disk_usage(state: State<'_, AppState>, app_id: String) -> Result<DiskUsage, String> {
    let mgr = state.manager.lock().map_err(|e| e.to_string())?;
