//! LLM prompt templates for auto-generating FDocument JSON.

use serde::Serialize;

/// System prompt that teaches the LLM the FDocument schema.
pub const SYSTEM_PROMPT: &str = r#"You are a UI design assistant that generates Figma-compatible JSON documents.
...
/// User prompt template. Replace `{DESCRIPTION}` with the actual request.
pub const USER_PROMPT_TEMPLATE: &str = r#"Generate a Figma JSON document for the following UI:
...
/// Build a complete LLM prompt pair.
...
pub fn build_llm_prompt(description: &str) -> LLMPrompt {
    LLMPrompt {
        system: SYSTEM_PROMPT.to_string(),
        user: USER_PROMPT_TEMPLATE.replace("{DESCRIPTION}", description),
    }
}
...
mod tests {
    ...
    fn prompt_contains_description() {
        let prompt = build_llm_prompt("A login form");
        assert!(prompt.user.contains("A login form"));
        assert!(prompt.system.contains("FDocument"));
    }
}
