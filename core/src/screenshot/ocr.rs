...
pub async fn extract_text(image: &DynamicImage) -> Result<String> {
    ...
    writer.StoreAsync().context("Failed to store stream")?.get().context("Failed to await store")?;
    ...
    let lang = Language::CreateLanguage(&HSTRING::from("en-US")).context("Failed to create lang")?;
    let engine = OcrEngine::TryCreateFromLanguage(&lang).context("Failed to create OcrEngine")?;

    let result = engine.RecognizeAsync(&bitmap).context("Failed to start recognize")?.get().context("Failed to await recognize")?;
